...
def generate_response(prompt, agent_type="generate", mode="standard"):
    ...
    agent_prompts = {
        ...
        "web": {
            "standard": f"""You are a Web Research Agent. Simulate the results of a web search for the following query:

            Query: {prompt}

            Generate a realistic web search result that includes:
            - A title for the result
            - Author/source information
            - Publication date
            - Key findings or content
            - Relevant data or statistics

            Format your response to look like a real web page result:""",

            "quick": f"""Simulate a short web result for: {prompt}
    ...
    # Get the appropriate prompt based on agent type and mode
